...
This document provides data flow diagrams and descriptions for the Finity platform, showing how user data flows through the system from collection to display. These diagrams support security architecture, compliance documentation, and development planning.
...
```
User → Finity Platform → Data Aggregation → Storage → Processing → Display → User
```
...
Cloud SQL (tier-specific windows)
  ...
  │     └─→ Writes Parquet features to Cloud Storage (gs://finity-ml-features/dt=YYYY-MM-DD/)
  │
...
- **Essential Tier:** Transactions live in Cloud SQL table `ledger_hot_essential` for 30 days. The `essential-ledger-archiver` job copies aged rows into Coldline (`gs://finity-ledger-archive/essential/<uid>/<YYYY>/<MM>`) before pruning the hot table.
