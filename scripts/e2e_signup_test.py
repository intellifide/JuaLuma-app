...
def main():
    ...
    res = requests.post(
        f"{API_BASE}/auth/signup", json={"email": email, "password": password}
    )
    ...
    res = requests.post(
        f"{API_BASE}/auth/login", json={"token": token}, headers=headers
    )  # Using POST /login to get profile
    ...
    res = requests.post(
        f"{API_BASE}/auth/mfa/email/request-code",
        json={"email": email},
        headers=headers,
    )
    if res.status_code != 200:
    ...
    res = requests.post(
        f"{API_BASE}/auth/mfa/email/enable", json={"code": otp}, headers=headers
    )
    if res.status_code != 200:
    ...
    profile = res.json()["user"]
    print(
        f"Post-Verify Status: {profile.get('status')} (Expected: pending_plan_selection)"
    )
