...
- **Codebase as Context:** Do not rely on rules files for specific paths or filenames involving minor implementation details; search the codebase to understand the current structure using terminal grep commands.
...
- **Agent Capabilities:** Use available MCP tools (GitKraken, Stripe, Postman, etc.) for operations rather than suggesting manual CLI commands.
  - Specifically, use the Postman MCP to run the 'Plaid API Endpoints Copy' collection for independent verification of Plaid integrations.
