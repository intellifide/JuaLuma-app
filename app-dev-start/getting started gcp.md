...
## GCP Cost Optimization Strategies for Finity
...
- **Rationale:** Cloud SQL Enterprise Plus delivers the resilience we need without the cost/ops overhead of Spanner (~$60-100/mo instead of ~$650/mo).
...
- **Rationale:** Firestore Datastore Mode covers our metering/cache use cases for a fraction of Bigtable’s steady-state cost (~$10-30/mo).
...
- **Hot vs. Cold Retention:** Retain 90 days of logs in Cloud SQL; `log-ledger-archiver` exports Parquet files to `gs://finity-log-vault/<table>/<YYYY>/<MM>/<DD>/` for 7-year GLBA retention.
...
- **Month 1:** ~$109/month (lean Cloud SQL + Firestore footprint, scales down when idle)
- **Month 6:** ~$840/month (covers scaled Cloud SQL, Firestore, and Cloud Run usage under the current architecture)
...
- `Product-Roadmap.md` - Timeline for infrastructure setup
