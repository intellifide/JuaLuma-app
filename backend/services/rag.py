...
from backend.models import get_session, Transaction
    ...
    try:
        ...
        try:
            ...
            stmt = select(Transaction).filter(
                ...
                Transaction.embedding.cosine_distance(query_embedding)
            ).limit(5)
            ...
            for tx in results:
                ...
                context_lines.append(f"- {tx.ts.strftime('%Y-%m-%d')}: {desc_text} ({tx.amount} {tx.currency})")
