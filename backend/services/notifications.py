...
class NotificationService:
    ...
    def create_notification(
        self, user: User, title: str, message: str, send_email: bool = True
    ) -> LocalNotification:
        """
        ...
        notification = LocalNotification(
            uid=user.uid, title=title, message=message, is_read=False
        )
        ...
        if send_email:
            try:
                ...
                logger.error(
                    f"Failed to dispatch email for notification {notification.id}: {e}"
                )
