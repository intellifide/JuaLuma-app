...
def create_support_agent(email, password, verbose=True):
    ...
    try:
        ...
        try:
            ...
            if verbose:
                print(f"User {email} found in Firebase (uid: {user_record.uid}). Updating password...")
    ...
    try:
        ...
        if not user:
            ...
            if user.uid != user_record.uid:
                print(f"WARNING: DB UID {user.uid} does not match Firebase UID {user_record.uid}. Updating DB to match Firebase.")
                user.uid = user_record.uid
            user.role = "support_agent"
        
        db.commit()
        if verbose:
            print(f"Successfully created/updated support agent {email} with role 'support_agent'")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--password", help="Agent password (optional, strong default used if omitted)")
