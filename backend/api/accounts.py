...
class AccountCreate(BaseModel):
    ...
    assigned_member_uid: str | None = Field(
...
def _build_web3_merchant_name(txn: dict, account: Account | None = None) -> str | None:
    """
    Build a user-friendly merchant name for Web3 transactions.
...
def _build_web3_description(txn: dict) -> str | None:
    """
    Build a user-friendly description for Web3 transactions.
...
def create_manual_account(
    ...
    if payload.assigned_member_uid:
        allowed_uids = get_household_member_uids(db, current_user.uid)
        if payload.assigned_member_uid not in allowed_uids:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
