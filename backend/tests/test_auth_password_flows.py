...
def test_change_password_mfa_required_but_missing(test_client: TestClient, test_db, mock_auth):
    """
...
def test_reset_password_mfa_required(test_client: TestClient, test_db):
    ...
    user = User(uid="mfa_user", email="mfa@example.com", role="user", mfa_enabled=True, mfa_secret="JBSWY3DPEHPK3PXP")
    test_db.add(user)
    test_db.commit()
...
def test_reset_password_mfa_success(test_client: TestClient, test_db):
    ...
    """
    secret = pyotp.random_base32()
    user = User(uid="mfa_user_2", email="mfa2@example.com", role="user", mfa_enabled=True, mfa_secret=secret, mfa_method="totp")
    test_db.add(user)
    test_db.commit()
    
    valid_code = pyotp.TOTP(secret).now()
    
    with patch("backend.api.auth.generate_password_reset_link", return_value="http://reset-link.com"):
