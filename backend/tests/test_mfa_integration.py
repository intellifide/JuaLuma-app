...
def test_mfa_complete_flow(mock_get_email_client, mock_verify_token_api, mock_verify_token_middleware, test_client, test_db):
    # Setup mocks
    ...
    # Bad Code -> 401
    res = test_client.post("/api/auth/login", json={"token": "fake_token", "mfa_code": "000000"})
    assert res.status_code == 401
    
    # Good Code -> 200
    code = totp.now()
    res = test_client.post("/api/auth/login", json={"token": "fake_token", "mfa_code": code})
    assert res.status_code == 200
    ...
    # ---------------------------------------------------------
    code = totp.now()
    res = test_client.post("/api/auth/mfa/disable", json={"code": code}, headers=headers)
    assert res.status_code == 200
    ...
    res = test_client.post("/api/auth/mfa/email/request-code", json={"email": test_email})
    assert res.status_code == 200
    ...
    res = test_client.post("/api/auth/mfa/email/enable", json={"code": email_code}, headers=headers)
    assert res.status_code == 200
    ...
    # Login with code
    res = test_client.post("/api/auth/login", json={"token": "fake_token", "mfa_code": login_code})
    assert res.status_code == 200
