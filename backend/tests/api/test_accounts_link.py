...
def test_link_web3_account_invalid_address(test_client: TestClient, mock_auth):
    ...
    response = test_client.post("/api/accounts/link/web3", json=payload)
    assert response.status_code == 400 # Validation error (converted to 400 by custom handler)
...
def test_link_web3_duplicate(test_client: TestClient, test_db, mock_auth):
    ...
    # First link
    test_client.post("/api/accounts/link/web3", json={
        "address": valid_address, 
        "chain_id": 1, 
    ...
    response = test_client.post("/api/accounts/link/web3", json={
        "address": valid_address, 
        "chain_id": 1, 
