...
const PLAN_FEATURE_OVERRIDES: Record<string, string[]> = {
  free: [
    ...
    'Supported uploads: docs, sheets, slides, images, JSON/XML (unsupported files blocked)',
...
const comparisonRows = [
  ...
  { label: 'AI Model Routing', free: 'gpt-oss-120b', essential: 'gemini-2.5-flash', pro: 'gemini-2.5-flash', ultimate: 'gemini-2.5-flash', icon: Zap },
  { label: 'Paid Exhaustion Fallback', free: 'N/A', essential: 'gpt-oss-120b', pro: 'gpt-oss-120b', ultimate: 'gpt-oss-120b', icon: Shield },
  { label: 'Supported AI Uploads', free: 'Yes', essential: 'Yes', pro: 'Yes', ultimate: 'Yes', icon: Database },
...
const faqs = [
  ...
  {
    q: 'How does AI model fallback work?',
    a: 'Free usage runs on `gpt-oss-120b`. Paid usage defaults to `gemini-2.5-flash`. If paid premium capacity is exhausted for the period, requests automatically fall back to `gpt-oss-120b` with explicit product messaging.',
  },
  {
    q: 'What files can I upload for AI context?',
    a: 'You can upload supported docs, sheets, slides, images, and JSON/XML files. Unsupported file types are blocked client-side where possible and rejected server-side if bypassed.',
  },
  {
      ...
      {loading ? (
        ...
        <>
            ...
            {displayPlans.map((plan, index) => {
              ...
              return (
                ...
                >
                  ...
                  <AppAuthLink
                    appPath={`/signup?plan=${planCode}${plan.interval === 'year' ? '_annual' : ''}`}
                    className={isFeatured ? 'btn w-full' : 'btn btn-secondary w-full'}
                  >
                    {getButtonText(plan)}
                  </AppAuthLink>
                </motion.article>
          ...
          <section className="text-center glass-panel">
            ...
            <AppAuthLink appPath="/signup" className="btn btn-lg">Create Your Free Account</AppAuthLink>
          </section>
        </>
      )}
    </div>
